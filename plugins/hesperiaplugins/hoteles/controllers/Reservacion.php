...
class Reservacion extends Controller
{
    ...
    public function onAddSeleccion(){
        ...
        $ups_result = Upselling::select(["titulo", "id", "tipo_inventario", "sumable", "ind_calendario", "disponible", "aplica_impuestos"])->whereIn("id", $ids_upsellings)->get();
        ...
        */
        foreach ($var_iniciales["hoteles"] as $hotel ) {
          if ($hotel["id"] == $busqueda["hotel"]) {
            foreach ($hotel["impuestos"] as $impuesto) {
              if($impuesto["moneda_id"]== $busqueda["moneda"]){
    ...
    public function onAprobar($id){
      ...
      if ($reservacion->aprobar($pago_insite)) {
      ...
      if (!empty($form["fechas"])) {
        ...
        foreach ($var_iniciales["hoteles"] as $hotel ) {
          if ($hotel["id"] == $form["Reservacion"]["hotel"]) {
            foreach ($hotel["impuestos"] as $impuesto) {
              if($impuesto["moneda_id"]== $form["Reservacion"]["moneda"]){
                array_push($impuestos, $impuesto);
              }
            }
            //$impuestos = $hotel["impuestos"];
    ...
    public function onGuardarReservacion(){
      ...
      if ($tiene_habitacion) {
        ...
        $backUser = BackendAuth::getUser();

        if($backUser->role->code=="freelance"){
          $origen = 3;
        }else{
          if($data["status"] != 2){
