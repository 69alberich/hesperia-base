...
class ResumenReserva extends ComponentBase{
  ...
  public function onRun(){
    ...
    if ($this->param('reserva_id')) {
      ...
      if ($reservacion) {
        ...
        || $reservacion->status_id == 1 || $reservacion->status_id == 6) {
        ...
        if (count($seleccion) > 0 && $total >0 ) {
          ...
          */
          ...
          if($reservacion->codigo != ""){
            return Redirect::to("pago-de-reserva/".$reservacion->codigo);
          }else{
  ...
  public function onPayInsite(){
    ...
    try {
      ...
      $resumen, function($message) use ($reserva, $resumen) {
      $emails = $reserva->hotel->emails_notificacion;
      foreach ($emails as $email) {
      $message->to($email["email"], $name=null);
      }
